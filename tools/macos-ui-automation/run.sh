...
usage() {
    printf '%s\n' \
        ...
        'Scenarios: onboarding, onboarding-busy, issues, rich-content, comment-confirmation, updates, team, settings' >&2
}
...
scenarios='onboarding onboarding-busy issues rich-content comment-confirmation updates team settings'
if [ "$list" -eq 1 ]; then
    if [ -n "$scenario" ] || [ "$suite" -eq 1 ] || [ "$self_test" -eq 1 ] || [ -n "$artifact_dir" ]; then
        usage
        exit 2
    fi
    printf '%s\n' onboarding onboarding-busy issues rich-content comment-confirmation updates team settings
    exit 0
fi
...
if [ "$suite" -eq 1 ]; then
    failures=0
    for name in onboarding onboarding-busy issues rich-content comment-confirmation updates team settings; do
