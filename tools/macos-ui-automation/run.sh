...
usage() {
    printf '%s\n' \
        ...
        'Scenarios: onboarding, onboarding-busy, issues, rich-content, updates, team, settings' >&2
}
...
scenarios='onboarding onboarding-busy issues rich-content updates team settings'
if [ "$list" -eq 1 ]; then
...
if [ "$suite" -eq 1 ]; then
    failures=0
    for name in onboarding onboarding-busy issues rich-content updates team settings; do
